...
TEST_CASE("reports error when current jumps abruptly - Current sensor") 
{
  ...
  REQUIRE(validateSensorReadings(currentReadings, numOfCurReadings, SensorProperties[CURRENT]) == NOISY);
}
...
TEST_CASE("noise less readings - Current sensor") 
{
  ...
  int numOfCurReadings = sizeof(currentReadings) / sizeof(currentReadings[0]);
  REQUIRE(validateSensorReadings(currentReadings, numOfCurReadings, SensorProperties[CURRENT]) == NOISE_FREE);
}
...
{
  double socReadings[] = {0.0, 0.01, 0.1, 0.51};
